...
import { getShopInfo } from "./settings.server";
...
const SYSTEM = (data: unknown, currency: string) =>
...
- All money figures in the data are in the store's currency${currency ? ` (${currency})` : ""}. Write prices in that currency; NEVER use $ or USD unless that IS the store currency.
...
export async function askAnalyst(
  ...
  const [data, shopInfo] = await Promise.all([
    getAnalyticsBundle(shopId),
    getShopInfo(shopId).catch(() => ({}) as { currencyCode?: string }),
  ]);
  const histText = history.slice(-6).map((m) => `${m.role}: ${m.content}`).join("\n");

  const out = await toolCall<{ answer: string; chartDataset?: Dataset }>({
    model: SONNET,
    system: SYSTEM(data, shopInfo.currencyCode ?? ""),
