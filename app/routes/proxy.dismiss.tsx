// Proactive dismissal handler (spec §9.2). A single dismissal silences proactive
// popups for the rest of the session. Behind the App Proxy (shop verified).
import type { ActionFunctionArgs } from "react-router";
import { z } from "zod";
import { authenticate } from "../shopify.server";
import { onDismiss } from "../intent/proactive.server";
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  await onDismiss(session.shop, body.sessionId);
