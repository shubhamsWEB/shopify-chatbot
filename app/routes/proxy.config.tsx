...
import { getSettings, DEFAULT_WELCOME } from "../intent/settings.server";
import { trialExpired, freshBackofficeMeta } from "../intent/billing.server";

export async function loader({ request }: LoaderFunctionArgs) {
  const { session } = await authenticate.public.appProxy(request);
  if (!session?.shop) return new Response("Unauthorized", { status: 401 });
  const backoffice = await freshBackofficeMeta(session.shop);
